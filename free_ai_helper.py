...
class FreeAIHelper:
    ...
    def _try_online_ai(self, question, chart_data=None, mai_hoa_data=None, luc_hao_data=None, topic=None,
        ...
        try:
            ...
            if offline_analysis_data:
                ...
                offline_ctx += (
                    f"\n=== DỮ LIỆU OFFLINE (Python, chính xác 100%) ===\n"
            ...
            if any(kw in q_lower_online for kw in ['cái gì','loại gì','sản xuất gì','làm gì','mặt hàng','sản phẩm gì','buôn bán gì','kinh doanh gì','nghề gì','ngành gì','là gì','thuộc loại','hình dạng','màu gì','chất liệu','tên gì','ai vậy','người nào','giống gì','như thế nào','trông như','nó là gì','loại nào','mẫu gì','kiểu gì','thể loại']):
                ...
                question_type_instruction = "\n⚠️ PHẢI MÔ TẢ đồ vật/sản phẩm/người dựa trên Ngũ Hành + Vạn Vật. KHÔNG phán CÁT/HUNG.\n"
                ...
                question_type_instruction = "\n⚠️ PHẢI TRẢ LỜI CON SỐ cụ thể.\n"
            else:
                question_type = 'YESNO'
                question_type_label = 'CÂU HỎI CÓ/KHÔNG hoặc TỔNG QUÁT'
                question_type_instruction = ''
            
            # V28.4: BUILD ANSWER KEY
            od = offline_analysis_data or {}
            v22 = od.get('v22_unified_strength', {})
            hanh_dt = v22.get('hanh_dt', '?')
            hanh_vat = v22.get('hanh_vat', {})
            vv_cu_the = v22.get('van_vat_cu_the', {})
            tier_cap = v22.get('tier_cap', '?')
            ngu_khi = v22.get('ngu_khi', '?')
            ts_stage_ak = v22.get('ts_stage', '?')
            dung_than_ak = od.get('dung_than', '?')
            
            ak = []
            ak.append(f"DT: {dung_than_ak} | Hành: {hanh_dt} | Tier: {tier_cap} | Ngũ Khí: {ngu_khi} | 12TS: {ts_stage_ak}")
            ak.append(f"Verdicts: KM={od.get('ky_mon_verdict','?')} LH={od.get('luc_hao_verdict','?')} MH={od.get('mai_hoa_verdict','?')} LN={od.get('luc_nham_verdict','?')} TA={od.get('thai_at_verdict','?')}")
            
            if question_type in ('WHAT', 'WHERE'):
                if hanh_vat:
                    ak.append(f"Hành {hanh_dt}: Hình={hanh_vat.get('hinh','?')} Chất={hanh_vat.get('chat_lieu','?')} Màu={hanh_vat.get('mau','?')} Hướng={hanh_vat.get('huong','?')}")
                if vv_cu_the:
                    ak.append(f"Vạn Vật: Đồ={vv_cu_the.get('do_vat','?')} Nhà={vv_cu_the.get('nha_cua','?')} Người={vv_cu_the.get('nguoi','?')}")
                if od.get('v18_detective'):
                ...
                _vd = [od.get('ky_mon_verdict',''), od.get('luc_hao_verdict',''), od.get('mai_hoa_verdict',''), od.get('luc_nham_verdict',''), od.get('thai_at_verdict','')]
            ...
            deep_prompt = (
                f"<system_role>\n"
                f"BẠN LÀ THIÊN CƠ ĐẠI SƯ — BẬC THẦY HUYỀN HỌC.\n"
                f"Kết hợp 6 PP: Kỳ Môn + Lục Hào + Mai Hoa + Thiết Bản + Đại Lục Nhâm + Thái Ất.\n"
                f"NGUYÊN TẮC: Trả lời ĐÚNG TRỌNG TÂM câu hỏi. Dùng DỮ LIỆU OFFLINE.\n"
                f"</system_role>\n\n"
                
                f"<question_type>\nLOẠI: **{question_type_label}**\n{question_type_instruction}</question_type>\n\n"
                ...
                f"═══ SƠ ĐỒ 1: CÂU HỎI CÓ/KHÔNG (YES/NO) ═══\n"
                f"PP GỐC: Lục Hào → Kỳ Môn xác nhận\n"
                f"[Nguyệt Lệnh] →sinh/khắc→ [DT Vượng/Suy] ←sinh/khắc← [Nhật Thần]\n"
                f"                                ↑ sinh                    ↑ khắc\n"
                f"                          [Nguyên Thần]              [Kỵ Thần] ←sinh← [Cừu Thần]\n"
                f"                                                         ↓\n"
                f"                                              KT+NT cùng động?\n"
                f"                                          CÓ → ⚡THAM SINH VONG KHẮC → CÁT\n"
                f"                                          KHÔNG → KT khắc DT → HUNG\n"
                f"    [DT Tuần Không?] → CÓ → chưa thành, chờ xuất Không\n"
                f"    [DT Nguyệt Phá?] → CÓ → phá hoại, không được\n"
                f"    [Thế↔Ứng] → Thế khắc Ứng = THẮNG, Ứng khắc Thế = THUA\n"
                f"    → KẾT LUẬN: DT Vượng+Nguyệt sinh+không Tuần Không = CÓ/ĐƯỢC\n"
                f"    → KẾT LUẬN: DT Suy+Nguyệt khắc+Tuần Không = KHÔNG/KHÔNG ĐƯỢC\n"
                f"    Kỳ Môn xác nhận: Cung BT↔Cung SV → BT khắc SV = mình thắng\n\n"
                
                f"═══ SƠ ĐỒ 2: CÂU HỎI TUỔI / SỐ LƯỢNG ═══\n"
                f"PP GỐC: Thiết Bản + Mai Hoa\n"
                f"[Bát Quái số] → Khảm=1, Khôn=2, Chấn=3, Tốn=4, Trung=5, Càn=6, Đoài=7, Cấn=8, Ly=9\n"
                f"    ↓\n"
                f"[Thượng Quái × Hạ Quái] → tạo phạm vi số\n"
                f"    ↓\n"
                f"[answer_key: TUỔI TRA SẴN] → lấy trung bình các PP đã tính\n"
                f"    → KẾT LUẬN: 'Khoảng X tuổi' (số cụ thể, KHÔNG phân tích CÁT/HUNG)\n"
                f"    CẤM: nói 'không thể xác định', phân tích 5 PP, liệt kê CÁT/HUNG\n\n"
                
                f"═══ SƠ ĐỒ 3: CÂU HỎI CÁI GÌ / Ở ĐÂU ═══\n"
                f"PP GỐC CÁI GÌ: Mai Hoa + Vạn Vật\n"
                f"[Thể Quái] → Bát Quái Tượng → Vật/Người/Chất liệu/Hình dạng/Màu\n"
                f"[Dụng Quái] → Tượng đối phương/sự việc\n"
                f"[Hỗ Quái] → Tượng ẩn bên trong\n"
                f"    → KẾT LUẬN: Mô tả vật thể/người cụ thể\n\n"
                f"PP GỐC Ở ĐÂU: Kỳ Môn + Đại Lục Nhâm\n"
                f"[Cung DT] → Phương hướng: 1=Bắc, 2=Tây Nam, 3=Đông, 4=ĐôngNam, 6=TâyBắc, 7=Tây, 8=ĐôngBắc, 9=Nam\n"
                f"[Mạt Truyền Chi] → Bát Quái → Phương bổ sung\n"
                f"    → KẾT LUẬN: 'Ở phương X' (phương cụ thể)\n\n"
                
                f"═══ SƠ ĐỒ 4: CÂU HỎI KHI NÀO ═══\n"
                f"PP GỐC: Đại Lục Nhâm + Lục Hào\n"
                f"[Sơ Truyền] = QUÁ KHỨ → đã xảy ra gì\n"
                f"[Trung Truyền] = HIỆN TẠI → đang diễn ra gì, Tuần Không? → chưa thành\n"
                f"[Mạt Truyền] = KẾT QUẢ → sẽ ra sao, Chi nào → ứng vào tháng/ngày chi đó\n"
                f"    ↓\n"
                f"Lục Hào Ứng Kỳ:\n"
                f"  DT Tĩnh+Vượng → ứng nhanh, chi DT tháng/ngày\n"
                f"  DT Tĩnh+Suy → ứng chậm, chờ Vượng mùa\n"
                f"  DT Động → ứng vào chi hào biến tháng/ngày\n"
                f"  DT Tuần Không → chờ xuất Không (chi Tuần Không qua)\n"
                f"    → KẾT LUẬN: 'Khoảng tháng X / ngày Y'\n\n"
                
                f"═══ SƠ ĐỒ 5: CÂU HỎI TỔNG QUÁT ═══\n"
                f"PP GỐC: Tổng hợp Lục Hào + Kỳ Môn + Thái Ất\n"
                f"[5 Verdicts: KM, LH, MH, LN, TA] → Đếm CÁT vs HUNG\n"
                f"  ≥4 CÁT → ĐẠI CÁT: rất thuận lợi\n"
                f"  ≥3 CÁT → CÁT: thuận lợi\n"
                f"  2 CÁT / 2 HUNG → LỠ CỠ: cân nhắc kỹ\n"
                f"  ≥3 HUNG → HUNG: bất lợi\n"
                f"  ≥4 HUNG → ĐẠI HUNG: rất xấu\n"
                f"    → KẾT LUẬN: dựa trên DT vượng/suy + đa số verdicts\n\n"
                
                f"CÁCH DÙNG SƠ ĐỒ:\n"
                f"1. Xem <question_type> → chọn SƠ ĐỒ phù hợp\n"
                f"2. Đọc factors trong <data> → điền vào từng ô trong sơ đồ\n"
                f"3. Theo mũi tên → rút ra KẾT LUẬN\n"
                f"4. Viết câu trả lời TRỰC TIẾP ≤300 chữ, trích yếu tố cụ thể\n"
                f"CẤM: liệt kê từng PP riêng lẻ, bịa %, tự tạo format mới, viết quá 300 chữ\n"
                f"</reasoning_protocol>\n\n"
    ...
    def answer_question(self, question, chart_data=None, topic=None, selected_subject=None, mai_hoa_data=None, luc_hao_data=None, **kwargs):
        ...
        if online_result:
            ...
            # ═══════ TRẢ LỜI THEO LOẠI ═══════
            # V28.1: CÂU HỎI CỤ THỂ (CÁI GÌ?) → dùng Vạn Vật + Thám Tử
            if is_what:
                final_parts.append(f"### 🔍 CÂU TRẢ LỜI CỤ THỂ (LOẠI/VẬT/NGƯỜI)")
                # Lấy Ngũ Hành DT → mapping vật chất
                _hanh_vat = NGU_HANH_VAT_CHAT.get(hanh_dt_v22, {})
                _vv_cu_the = _get_van_vat_cu_the(hanh_dt_v22, unified_v22.get('tier_key', 'TRUNG_BÌNH') if unified_v22 else 'TRUNG_BÌNH')
                if _hanh_vat:
                    final_parts.append(f"**Ngũ Hành:** {hanh_dt_v22} → Hình dạng: {_hanh_vat.get('hinh', '?')} | Chất liệu: {_hanh_vat.get('chat_lieu', '?')} | Màu: {_hanh_vat.get('mau', '?')}")
                    final_parts.append(f"**Hướng:** {_hanh_vat.get('huong', '?')} | **Vị:** {_hanh_vat.get('vi', '?')} | **Cơ thể:** {_hanh_vat.get('co_the', '?')}")
                if _vv_cu_the:
                    final_parts.append(f"**Đồ vật cụ thể:** {_vv_cu_the.get('do_vat', '?')}")
                    final_parts.append(f"**Nhà cửa:** {_vv_cu_the.get('nha_cua', '?')}")
                    final_parts.append(f"**Người liên quan:** {_vv_cu_the.get('nguoi', '?')}")
                # Lấy V18 Thám Tử nếu có
                if v18_detective:
                    final_parts.append(f"\n**🕵️ Thám Tử Lắp Ghép:**")
                    for line in v18_detective.split('\n')[:10]:
                        if line.strip():
